...
✅ **Etapa 4 feita em 10/09/2026.** `naoapego.com.br` (GitHub Pages) lê `v_catalogo` do Supabase em vez do CSV com cache de 5min — mesma lógica de exibição (drops, tags, fotos do Drive), zero mudança de regra de negócio. No caminho, achamos e corrigimos um bug real de permissões que nunca tinha sido testado: a view `v_catalogo` (`security_invoker=true`) fazia `p.*` e o `anon` só tinha grant em 3 colunas de `closets`, nada em `pecas` — a view sempre teria devolvido "permission denied" pra qualquer visitante do site, só que ninguém tinha testado isso com a chave pública até agora. Corrigido: a view lista só as colunas seguras de expor (nunca split/reserva/origem de cadastro), com GRANT exatamente nessas colunas + `closet_id` (usado só no JOIN). Testado de ponta a ponta com Chrome real, inclusive comparando com a produção antiga (mesmo comportamento de placeholder de foto quando a peça não tem foto real — não é regressão, é assim hoje também).

✅ **Melhoria no cadastro (Etapa 3) feita em 10/09/2026**: fluxo em lote em `/painel/cadastro/lote` — ela seleciona de uma vez todas as fotos de uma sessão (várias peças), o sistema separa por peça automaticamente (pausa de tempo entre fotos) e ela ajusta numa tela de revisão (separar/unir grupo, reordenar, descartar) antes de cadastrar peça por peça, reaproveitando o mesmo formulário com IA/ditado de sempre. Resolve a fricção relatada de selecionar fotos boas uma peça de cada vez. Testado de ponta a ponta com peça real (criada e limpa dos dois lados). "Foto profissional via IA" ficou de fora por ora — decisão explícita, o Henrique pretende usar outro modelo focado em imagem pra essa parte depois.

✅ **Etapa 5 iniciada em 10-11/09/2026 (agente de vendas via WhatsApp)**: em vez de um formulário web pra registrar venda, o Henrique propôs um agente conversacional — a Luiza manda "vendi a BR2510008 pra Maria, 129 pix" pro WhatsApp da própria loja (o número que já atende cliente), a IA extrai os dados, confirma mostrando a peça de verdade, e só grava depois do "sim" dela. Resolve o problema real que motivou a etapa: ela é ruim de registro em caderno, e o repasse de fim de mês depende de datas certas. Código em `nao-apego-app/bot-vendas/` (canal-agnóstico: `core.mjs` faz a lógica, `whatsapp.mjs` é o adaptador ativo, `telegram.mjs` fica pronto como alternativa). Testado em produção com venda real (registrada e revertida depois, Supabase + Sheets). Roda sob `pm2` no Mac do Henrique — **ainda não é definitivo**: falta migrar pra um servidor sempre ligado antes de confiar 100%, e por pelo menos 1 ciclo de repasse continuar conferindo manualmente em paralelo antes de considerar o Supabase fonte de verdade pra venda/repasse. Descoberta técnica registrada em `bot-vendas/.env.local`: o WhatsApp às vezes endereça a mesma conversa por um "LID" de privacidade em vez do número puro — o filtro de autorização precisa aceitar os dois formatos.
